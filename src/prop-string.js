const getPropString = propData => {
  ...
  */
  propNames.forEach(name => {
    ...
    if (propData[name].type.name === "func") {
      return;
    }

    if (["null", "false"].includes(propData[name].value)) {
      if (
        propData[name].defaultValue &&
        propData[name].defaultValue.value === "true"
      ) {
    ...
    */
    if (
      propData[name].type.name === "bool" &&
      propData[name].value === "true"
    ) {
      propString += ` ${name}`;
      return true;
    }
    ...
    */
    ...
    if (
      propData[name].type.name === "string" &&
      propData[name].value.length > 2
    ) {
    ...
    if (propData[name].type.name === "enum") {
      ...
      */
      if (isNumber(propData[name].value)) {
        ...
        typeof propData[name].value === "string" &&
        propData[name].value.length > 0
      ) {
        propString += ` ${name}=${propData[name].value}`;
        return true;
      }
    ...
    */
    if (propData[name].value && propData[name].value !== '""') {
      console.log("default caser");
      propString += ` ${name}={${propData[name].value}}`;
      return true;
    }
  });
